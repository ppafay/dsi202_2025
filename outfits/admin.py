...
from .models import Outfit, Cart, CartItem, Category
...
class CategoryAdmin(admin.ModelAdmin):
    list_display = ('name', 'slug', 'outfit_count_display')
    search_fields = ('name', 'description')
    prepopulated_fields = {'slug': ('name',)}

    def outfit_count_display(self, obj):
...
class OutfitAdmin(admin.ModelAdmin):
    list_display = ('name', 'category', 'price_per_day_display', 'is_rented', 'id')
    list_filter = ('is_rented', 'category')
    search_fields = ('name', 'description', 'category__name')
    list_editable = ('is_rented', 'category')
...
class CartItemInline(admin.TabularInline):
    ...
    readonly_fields = (
        ...
        'rental_days_display', 'calculated_item_price', 'item_price_at_rental'
    )
    fields = (
        'outfit', 'outfit_link_inline', 'quantity',
        'start_date', 'end_date', 'outfit_price_per_day',
        'rental_days_display', 'item_price_at_rental', 'calculated_item_price'
    )
...
class CartAdmin(admin.ModelAdmin):
    list_display = (
        'id','user_display','total_price_display','is_paid',
        'rental_status','paid_at_display','created_at_display',
        'item_count_display','return_tracking_number'
    )
    ...
    readonly_fields = (
        ...
        'total_price_display_field','payment_slip_preview',
        'customer_uploaded_image_preview','return_shipment_image_preview'
    )
    fieldsets = (
        (None, {'fields': ('id', 'user', ('is_paid', 'paid_at'), 'payment_slip_preview', 'payment_slip')}),
        ('Rental Information', {'fields': (
            'rental_status','customer_uploaded_image_preview',
            'customer_uploaded_image','return_tracking_number',
            'return_shipment_image_preview','return_shipment_image'
        )}),
    ...
    actions = [
        'mark_as_payment_confirmed', 'mark_as_processing',
        'mark_as_shipped', 'mark_as_delivered',
        'mark_as_customer_returning', 'mark_as_returned_received',
    ...
    def user_display(self, o): return o.user.username if o.user else "Guest"
    user_display.short_description = 'ผู้ใช้งาน'

    def total_price_display(self, o):
        o.calculate_and_set_total_price()
        return f"฿{o.total_price:,.2f}"
    total_price_display.short_description = 'ยอดรวม'

    def total_price_display_field(self, o):
        o.calculate_and_set_total_price()
        return f"฿{o.total_price:,.2f}"
    total_price_display_field.short_description = 'ยอดรวม (คำนวณ)'

    def paid_at_display(self, o):
        return o.paid_at.strftime("%d %b %Y, %H:%M") if o.paid_at else "-"
    paid_at_display.short_description = 'ชำระเงินเมื่อ'

    def created_at_display(self, o):
        return o.created_at.strftime("%d %b %Y, %H:%M") if o.created_at else "-"
    created_at_display.short_description = 'สร้างเมื่อ'

    def item_count_display(self, o):
        return o.item_count
    item_count_display.short_description = 'จำนวนรายการ'

    def payment_slip_preview(self, o):
        if o.payment_slip:
            return format_html(f'<a href="{o.payment_slip.url}" target="_blank"><img src="{o.payment_slip.url}" width="100" height="100" style="object-fit:cover;border:1px solid #ddd;"/></a>')
        return "(No slip)"
    payment_slip_preview.short_description = 'สลิป'

    def customer_uploaded_image_preview(self, o):
        if o.customer_uploaded_image:
            return format_html(f'<a href="{o.customer_uploaded_image.url}" target="_blank"><img src="{o.customer_uploaded_image.url}" width="100" height="100" style="object-fit:cover;border:1px solid #ddd;"/></a>')
        return "(No image)"
    customer_uploaded_image_preview.short_description = 'รูปจากลูกค้า'

    def return_shipment_image_preview(self, o):
        if o.return_shipment_image:
            return format_html(f'<a href="{o.return_shipment_image.url}" target="_blank"><img src="{o.return_shipment_image.url}" width="100" height="100" style="object-fit:cover;border:1px solid #ddd;"/></a>')
        return "(No return image)"
    return_shipment_image_preview.short_description = 'รูปหลักฐานส่งคืน'
    ...
    def save_formset(self, request, form, formset, change):
        ...
        if isinstance(formset.instance, Cart):
...
class CartItemAdmin(admin.ModelAdmin):
    list_display = (
        ...
        'cart_link',
        'outfit_link',
        'outfit_image_preview',  # ✅ แสดงภาพ
        'quantity',
        'start_date_display',
        'end_date_display',
        'rental_days_display',
        'calculated_price_display',
        'item_price_at_rental_display_col'
    )
    ...
    def cart_link(self, o):
        if o.cart:
            l = reverse("admin:outfits_cart_change", args=[o.cart.id])
            ui = f" (User: {o.cart.user.username})" if o.cart.user else " (Guest)"
            return format_html(f'<a href="{l}">Cart #{o.cart.id}{ui}</a>')
        return "N/A"
    cart_link.short_description = 'ตะกร้า'
    cart_link.admin_order_field = 'cart__id'

    def outfit_link(self, o):
        if o.outfit:
            l = reverse("admin:outfits_outfit_change", args=[o.outfit.id])
            return format_html(f'<a href="{l}">{o.outfit.name}</a>')
        return "N/A"
    outfit_link.short_description = 'ชุด'
    outfit_link.admin_order_field = 'outfit__name'

    @admin.display(description="ภาพชุด")
    def outfit_image_preview(self, obj):
        if obj.outfit and obj.outfit.image and hasattr(obj.outfit.image, 'url'):
            return format_html(
                '<img src="{}" width="70" height="70" style="object-fit:cover; border-radius:8px; border:1px solid #ddd;" />',
                obj.outfit.image.url
            )
        return "-"

    def start_date_display(self, o):
        return o.start_date.strftime("%d %b %Y") if o.start_date else "-"
    start_date_display.short_description = 'เริ่มเช่า'
    start_date_display.admin_order_field = 'start_date'

    def end_date_display(self, o):
        return o.end_date.strftime("%d %b %Y") if o.end_date else "-"
    end_date_display.short_description = 'วันคืน'
    end_date_display.admin_order_field = 'end_date'

    def rental_days_display(self, o):
        return o.get_rental_days()
    rental_days_display.short_description = 'วัน'

    def calculated_price_display(self, o):
        return f"฿{o.get_total_item_price():,.2f}"
    calculated_price_display.short_description = 'ราคารวม (สด)'

    def item_price_at_rental_display_col(self, o):
        return f"฿{o.item_price_at_rental:,.2f}"
    item_price_at_rental_display_col.short_description = 'ราคาที่บันทึก'
    item_price_at_rental_display_col.admin_order_field = 'item_price_at_rental'

    def item_price_at_rental_display(self, o):
        return f"฿{o.item_price_at_rental:,.2f}"
    item_price_at_rental_display.short_description = 'ราคาที่บันทึกไว้สำหรับรายการนี้'
